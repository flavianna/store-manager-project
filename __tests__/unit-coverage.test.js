...
describe('Testes das camadas Model, Service e Controller', () => {
  ...
  describe('13 - Desenvolva testes que cubram no mínimo 30% de linhas e tenha no mínimo 8 funções escritas nas camadas da sua aplicação', () => {

    it('Será validado que em cada uma das pastas `models`, `services` e `controllers`, a cobertura total das linhas dos arquivos é maior ou igual a 30% e o número de funções escritas é maior ou igual a 8', async () => {
      assertCoveragePercentage(coverageResultsArr, 30, 8);

    });
  });

  describe('15 - Desenvolva testes que cubram no mínimo 35% de linhas e tenha no mínimo 9 funções escritas nas camadas da sua aplicação', () => {
  ...
  describe('17 - Desenvolva testes que cubram no mínimo 40% de linhas e tenha no mínimo 10 funções escritas nas camadas da sua aplicação', () => {
  ...
  describe('19 - Desenvolva testes que cubram no mínimo 50% de linhas e tenha no mínimo 10 funções escritas nas camadas da sua aplicação', () => {
    it('Será validado que em cada uma das pastas `models`, `services` e `controllers`, a cobertura total das linhas dos arquivos é maior ou igual a 50% e o número de funções escritas é maior ou igual a 10', async () => {
      assertCoveragePercentage(coverageResultsArr, 50, 10);
    });
  });

  describe('20 - Desenvolva testes que cubram no mínimo 60% de linhas e tenha no mínimo 10 funções escritas nas camadas da sua aplicação', () => {
    it('Será validado que em cada uma das pastas `models`, `services` e `controllers`, a cobertura total das linhas dos arquivos é maior ou igual a 60% e o número de funções escritas é maior ou igual a 10', async () => {
      assertCoveragePercentage(coverageResultsArr, 60, 10);
    });
  });
})
